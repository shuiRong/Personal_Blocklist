...
blocklist.searchpage.showAddBlocklistMessage = function (pattern, section) {
    ...
    showMessage.style.cssText =
        'font-size:15px;background:#d8f7eb;padding:30px;margin:20px 0;box-sizing:border-box;';
    ...
    cancelMessage.style.cssText =
        'cursor: pointer;margin-top:20px;font-size:16px;font-weight: 700; color: #0066c0;';
...
blocklist.searchpage.handleRemoveBlocklistFromSerachResult = function (
    response
) {
    if (response.blocklist != undefined) {
        blocklist.searchpage.blocklist = response.blocklist;
...
blocklist.searchpage.toggleSections = function (pattern, display) {
    var searchResultPatterns = document.querySelectorAll(
        blocklist.searchpage.SEARCH_RESULT_DIV_BOX
    );

    for (let i = 0, length = searchResultPatterns.length; i < length; i++) {
        var searchResultPattern = searchResultPatterns[i];
        var searchResultHostLink = searchResultPattern.querySelector(
            blocklist.searchpage.LINK_TAG
        );
        if (searchResultHostLink) {
            var HostLinkHref = searchResultHostLink.getAttribute('href');
            var sectionLink = HostLinkHref.replace(
                blocklist.common.HOST_REGEX,
                '$2'
            );
...
blocklist.searchpage.addBlocklistFromSearchResult = function (
    hostlink,
    searchresult
) {
...
blocklist.searchpage.insertAddBlockLinkInSearchResult = function (
    searchResult,
    hostlink
) {
    ...
    insertLink.style.cssText =
        'color:rgba(0,0,0,0.3);margin:0;margin-top:5px;cursor: pointer;width:fit-content;';
    searchResult.appendChild(insertLink);

    insertLink.addEventListener(
        'click',
        function () {
            blocklist.searchpage.addBlocklistFromSearchResult(
                hostlink,
                searchResult
            );
        },
...
blocklist.searchpage.modifySearchResults = function (parent_dom) {
    ...
    var searchResultPatterns = parent_dom.querySelectorAll(
        blocklist.searchpage.SEARCH_RESULT_DIV_BOX
    );
    ...
    for (let i = 0, length = searchResultPatterns.length; i < length; i++) {
        ...
        if (searchResultHostLink) {
            var HostLinkHref = searchResultHostLink.getAttribute('href');
            var HostLinkPattern = blocklist.common.getHostNameFromUrl(
                HostLinkHref
            );
...
blocklist.searchpage.initMutationObserver = function () {
    ...
    blocklist.searchpage.mutationObserver = new MutationObserver(function (
        mutations
    ) {
