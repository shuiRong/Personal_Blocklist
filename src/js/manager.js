...
blocklist.manager.createBlocklistPattern = function (pattern) {
    ...
    let patRowDeleteButton = $(
        '<div class="isBlocked" style="margin-right: 15px;"></div>'
    );
    let span = $(
        '<span style="color:rgba(0,0,0,0.3);margin:0;margin-top:5px;cursor: pointer;width:fit-content;">' +
    ...
    let patRowHostName = $('<div class="pattern-block">' + pattern + '</div>');
...
blocklist.manager.handleRefreshResponse = function (response) {
    ...
    if (response.blocklist != undefined && length > 0) {
        ...
        for (let i = 0; i < length; i++) {
            var patRow = blocklist.manager.createBlocklistPattern(
                response.blocklist[i]
            );
...
blocklist.manager.handlePwsOptionBox = function (response) {
    ...
    if (response.pws_option == 'on') $('#pws_option').prop('checked', true);
